...
class DCTFeatures:
    ...
    def DCT_reconstruction(self, dct_features, stat_features, coef_size):
        ...
        for i in range(num_of_phones):
            ...
            print stat_features[(i * stat_size) + 1], num_of_frames
    ...
    def DCT_decomposition_from_lab_file(self, phone, ph_arr, mean_f0_arr, f0_arr, decomposition_unit, coef_size, stat_size):
        ...
        while i < file_len:
            ...
            if stat_size == 10:
                ...
                dr_maxf0 = np.argmax(segment_contour) + 1
    ...
    def DCT_reconstruction_from_lab_file(self, phone, ph_arr, mean_f0_arr, interp_f0_arr, denorm_data, decomposition_unit, coef_size, out_dim, use_org_dur):
        ...
        while i < file_len: 
            ...
            if(phone[i] == '#'):
                sil_count = sil_count + 1
                #sil_frames = num_of_frames
                i = i + 1
                continue;
            if (decomposition_unit == 'word' or decomposition_unit == 'syllable'):
                if (phone[i] == 'pau'):
            ...
            #segment_contour = interp_f0_arr[ph_start-sil_frames:ph_end-sil_frames]; 
    ...
    def prepare_duration_list(self, lab_file, decomposition_unit, remove_silence=True):
        
        ### read lab file ###
        [phone, ph_arr, mean_f0_arr] = self.read_state_align_label_file(lab_file)
        ...
        while j < len(phone): 
            ...
            if(decomposition_unit=='word' and word_end):
                dur_list.append(wrd_num_of_frames)
                wrd_num_of_frames=0
    ...
    def find_mean_f0_contour(self, interp_f0_arr, phone, ph_arr, mean_f0_arr, decomposition_unit):
        ### To generate mean F0 contour ###
        mean_f0_contour = prosody_funcs.zeros(len(interp_f0_arr), 1)
        file_len = len(phone)
        i = 0;
        while i < file_len: 
            if(decomposition_unit == "phone"):
            ...
            segment_contour = interp_f0_arr[ph_start:ph_end];
            mean_f0_contour[ph_start: ph_end] = np.mean(segment_contour)
            i+=1
            
        mean_f0_contour[-7:] = np.mean(segment_contour)
            
        return mean_f0_contour    
    ...
    def interpolate_f0_data(self, f0_data):
        
        data    = [float(x) for x in f0_data]
        ip_data = data
...
if __name__ == "__main__":
    ...
    ### Outputs ###
    out_dir = os.path.join(work_dir, 'Data/inter-module/'+speaker+'/dct_features/' + feat_dir_path + '/')
    ...
    if DFP:
        ...
        if modifyorgF0:
            ...
            ### Outputs ###
            out_dir = '/afs/inf.ed.ac.uk/group/cstr/projects/phd/s1432486/work/LSTM_work/dnn_tts_blz16ehmm/lstm_rnn/data/sylmf0/'
            ...
            for k in range(len(list_arr)):
                ...
                ### processing lf0 file ###
                ori_f0_arr = io_funcs.load_float_file(f0_file)  
                interp_f0_arr = prosody_funcs.interpolate_f0(f0_file)
            
                ### read label file ###
                [phone, ph_arr, mean_f0_arr] = prosody_funcs.read_state_align_label_file(lab_file)
                mean_f0_contour = prosody_funcs.find_mean_f0_contour(interp_f0_arr, phone, ph_arr, mean_f0_arr, decomposition_unit)
                #prosody_funcs.plot_DBR(interp_f0_arr, mean_f0_contour)
                
                if remove_silence:
                    ph_start = int(ph_arr[0][1] / (np.power(10, 4) * 5));
                    ph_end = int(ph_arr[1][len(phone) - 2] / (np.power(10, 4) * 5));
                    ori_f0_data = ori_f0_arr[ph_start:ph_end]
                    interp_f0_data = interp_f0_arr[ph_start:ph_end]
                    mean_f0_data = mean_f0_contour[ph_start:ph_end]
                else:
                    ori_f0_data = ori_f0_arr
                    interp_f0_data = interp_f0_arr
                    mean_f0_data = mean_f0_contour
                
                ### apply original V/UV decision on mean f0 data ###
                #ori_f0_data = np.array(ori_f0_data)
                #mean_f0_data = np.array(mean_f0_data)
                #mean_f0_data[ori_f0_data==0] = 0
                
                ### plot ###
                #prosody_funcs.plot_DBR(interp_f0_arr, mean_f0_data)                
                
                ### To write mean F0 contour into a file ###
                out_file = os.path.join(out_dir, filename + '.lf0') 
                io_funcs.array_to_binary_file(mean_f0_data, out_file)
        ...
        if prosodyrecons:
            ...
            for k in range(len(list_arr)):
                ...
                #interp_bf0_data = prosody_funcs.interpolate_f0_data(base_f0_data)
